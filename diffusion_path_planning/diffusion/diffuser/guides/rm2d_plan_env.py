...
class RandMaze2DEnvPlanner(KukaEnvPlanner):
    ...
    def plan_an_env(self, env, policy, use_normed_wallLoc, plan_env_config:utils.Config, start_point, goal_point):
        """
        ...
        print(f"self.prob_permaze:{self.prob_permaze}")
        # list of numpy, extract start and target
        #print(f"problems_dict:{self.problems_dict}, maze_idx:{self.maze_idx}, prob_permaze:{self.prob_permaze}")
        #obs_start_list, target_list = kuka_obs_target_list(self.problems_dict, self.maze_idx, self.prob_permaze)
        obs_start_list =  np.array([start_point])
        target_list = np.array([goal_point])
        #print('\033[32m' + f"rm2d_plan_env 54 start_point:{[start_point]}, target_point:{[goal_point]}" + '\033[0m')
        print('\033[32m' + f"rm2d_plan_env 55 obs_start_list:{obs_start_list}, target_list:{target_list}" + '\033[0m')
        ...
        #print('\033[32m' + f"obs_start_arr:{obs_start_arr}, target_ar:{target_arr}" + '\033[0m')
    ...
    def option_1_pick_1_traj(self, samples, env, prob_permaze):
        ...
        for i_p in range(prob_permaze):
            ...
            for i_aj in range(self.samples_perprob):
                ...
                if is_valid:
                    ...
                    print('\033[31m' + f'prob{i_p} traj{i_aj}: success' + '\033[0m')
                    #print(f"modelout_path_list[{i_p}]:{modelout_path_list[i_p]}")
                    suc_list[i_p] = True
                    break
                else:
                    if i_aj == self.samples_perprob - 1:
                        print('\033[31m' + f"Problem{i_p} failed to generate trajectory." + '\033[0m')
        ...
        print('rm2d_plan_env 219 suc_list', suc_list)
